...
struct EditaPlantaFoto: View {
    ...
    var body: some View {
        NavigationStack{
            VStack{
                /*NavigationLink("ImagenPlanta", value: imagePicker)
                    .navigationDestination(for: ImagePicker) { item in
                        ImagePicker(show: $imagePicker, image: $fotoNueva, source: source)
                    }
                NavigationLink("Regreso", value: habitacion)
                    .navigationDestination(for: habitacion) { item in
                        Habitacion(habitacion: item)
                    }*/
                NavigationLink(destination: ImagePicker(show: $imagePicker, image: $fotoNueva, source: source), isActive: $imagePicker){
                    EmptyView()
                }.navigationBarHidden(true)
                NavigationLink(destination: Habitacion(habitacion: habitacion), isActive: $atras){
                    EmptyView()
                }.navigationBarHidden(true)
                ...
                .edgesIgnoringSafeArea(.all).navigationTitle("")
        }
    }
}
